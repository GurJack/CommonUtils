    ...
    public static class LoggerHandler
    {
        ...
        static void SetupNLogConfiguration()
        {
            try
            {
                ...
                var fileTarget = new FileTarget("fileTarget")
                {
                    ...
                    FileName = Path.Combine(GlobalConstant.AppDataPath, "Logs", "${shortdate}.log"),

                    // Макет сообщения
                    Layout = "${longdate}|${level:uppercase=true}|${logger}|${message}${exception:format=tostring}",

                    // Настройки ротации логов
                    ArchiveFileName = Path.Combine(GlobalConstant.AppDataPath, "Logs\\Archives", "{#}.log"),
        ...
        public static void Fatal(Exception ex, string contextMessage = "Ошибка в приложении", bool passTheErrorOn = true)
        {
            ...
            catch (Exception handlerEx)
            {
                ...
                _logger.Fatal($"Original error: {ex.GetType().Name}", ex);
